  ...
  <div class="success">
    ...
    <div class="main">
        ...
        <div class="order-info">
            ...
            <p class="info3"><b>课程信息：</b><span :key="key" v-for="course,key in course_list">《<router-link :to="'/courses/detail/'+course.id">{{course.name}}</router-link>》</span></p>
        </div>
    ...
    methods:{
        alipayResultHander(){
            ...
            this.$axios.get(`${this.$settings.HOST}/payments/alipay/result/`+location.search).then(response=>{
                ...
                // this.$router.go(-1); //这个是，如果订单查询失败的话，就继续返回支付宝查询结果
            })
        }
    },
    components:{
